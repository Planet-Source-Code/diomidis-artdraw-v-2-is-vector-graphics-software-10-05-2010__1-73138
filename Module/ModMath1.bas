...
'Υπολογισμός επιφάνειας με ανεξαρτητη την καθε πλευρά
...
'Υπολογισμός 4 σημείων σε δεδομένες συντεταγμένες και γωνίες
' υπολογιζει ολα τα κομμάτια των αλουμινίων - οχι επιφανειες
...
'             P1 = tp1
'             P2 = tp2
'             'Debug.Print "Με Πάτημα"
'             'Debug.Print Str(P1.X) + "," + Str(P1.Y) + "," + Str(P2.X) + "," + Str(P2.Y)
'             'Debug.Print Str(P3.X) + "," + Str(P3.Y) + "," + Str(P4.X) + "," + Str(P4.Y)
'         End If
         'Περιστροφή του τεμαχίου στο 0,0
         P_1 = Rotate(P1.X, P1.Y, AngOrig)  'ΟΚ
         P_2 = Rotate(P2.X, P2.Y, AngOrig)  'ΟΚ
         P_3 = Rotate(P3.X, P3.Y, AngOrig)  'ΟΚ
         P_4 = Rotate(P4.X, P4.Y, AngOrig)  'ΟΚ
...
'         'Μετακίνηση των σημείων στα Τελικά Σημεία
         P1.X = P_1.X + X1
         P1.Y = -(P_1.Y + Y1)
         P2.X = P_2.X + X1
         P2.Y = -(P_2.Y + Y1)
...
'        'Debug.Print Str(cX1) + "," + Str(cY1) + "," + Str(cX2) + "," + Str(cY2)
       ...
       P2.X = cX2
       P2.Y = cY2
       ''Debug.Print "ΒΗΜΑ", cX1, cY1, cX2, cY2
       'Stop
       'Περιστροφή του τεμαχίου στο 0,0
       P_1 = Rotate(P1.X, P2.Y, AngOrig) 'ΟΚ
       P_2 = Rotate(P2.X, P2.Y, AngOrig) 'ΟΚ
'      'Debug.Print "ROTATE Point "
'      'Debug.Print Str(P_1.X) + "," + Str(P_1.Y) + "," + Str(P_2.X) + "," + Str(P_2.Y)
'       'Μετακίνηση των σημείων στα Τελικά Σημεία
...
'υπολογισμος μήκους ταφ
'ελεγχο αν θέλει αλλαγή στα Υ
...
'υπολογισμος σημείων ταφ για σχεδίαση
'ελεγχο αν θέλει αλλαγή στα Υ
...
'
'       'Περιστροφή του τεμαχίου στο 0,0
'       P_1 = Rotate(P1, AngOrig) 'ΟΚ
'       P_2 = Rotate(P2, AngOrig) 'ΟΚ
'      'Debug.Print "ROTATE Point "
'      'Debug.Print Str(P_1.X) + "," + Str(P_1.Y) + "," + Str(P_2.X) + "," + Str(P_2.Y)
''       'Μετακίνηση των σημείων στα Τελικά Σημεία
'       P1.X = P_1.X + X1
...
'        m2 = SlopeLine(mX1, mY1, mX2, mY2)
        ...
        If (m1 <> 0 And m2 <> 0) And (m1 = m2) Then MsgBox "Παράλληλες", vbCritical: Exit Function
...
'2 γωνίες γνωστές - μια πλευρά γνωστή στις 90 μοίρες
